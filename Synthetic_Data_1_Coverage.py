...
filename1 = r'Synthetic_Data_1_Coverage '+ str(B) + r' ' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
...
# weighted features
W = {1:[2,6,19,45,98],0.75:[5,22,34,81,92],0.5:[7,11,28,56,85],0.25:[3,77,79,88,93]}
vec_w = np.zeros(100)
for i in W:
    for j in W[i]:
        vec_w[j] = i
...
for i in range (0,500):#create rows
    row = [random.uniform(-1, 1) for i in range(100)]# 100 features between [-1,1]
    r = np.asarray(row)
...
for B in f_num:
    ...
    msol=mdl.solve(TimeLimit=20000)
